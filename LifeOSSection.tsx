...
import { 
  ...
  CheckCircle2,
  LayoutGrid,
  Volume2, // Import Volume2 icon for play
  VolumeX, // Import VolumeX icon for stop
  Mic,     // Import Mic icon for speech-to-text
  MicOff   // Import MicOff icon (optional, could use VolumeX)
} from 'lucide-react';
import type { 
  AllReports, 
  IndividualBirthData, 
...
interface LifeOSSectionProps {
  ...
  speak: (text: string) => Promise<void>; // New prop
  isSpeaking: boolean; // New prop
}

export const LifeOSSection: React.FC<LifeOSSectionProps> = ({ individuals, allReports, speak, isSpeaking }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {view === 'quiz' && activeProfile && (
          <QuizWizard 
            ...
            isSpeaking={isSpeaking} // Pass isSpeaking prop
          />
        )}
...
function QuizWizard({ initialData, allReports, individuals, onCancel, onComplete, speak, isSpeaking }: QuizWizardProps) {
  ...
  if (step === 0) {
    return (
      <div className="max-w-xl mx-auto bg-slate-900/50 backdrop-blur-xl p-10 rounded-3xl border border-white/10 shadow-2xl animate-fade-in relative overflow-hidden">
        ...
        <h2 className="text-3xl font-bold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 relative z-10">User Initialization</h2>
      ...
      >
        ...
        <div className="flex-1 space-y-8 relative z-10">
          ...
          <div className="space-y-4 relative pr-8"> {/* Added pr-8 for speaker button */}
             <button
                onClick={() => speak(`${question.label}. ${question.text}`)}
                className={`absolute top-0 right-0 p-2 rounded-full transition-all duration-200 ${isSpeaking ? 'text-brand-accent animate-pulse' : 'text-slate-500 hover:text-emerald-400'}`}
                aria-label={`Listen to question: ${question.label}`}
                disabled={isSpeaking}
              >
...
function ProfileView({ profile, allReports, individuals, onBack, onDelete, speak, isSpeaking }: ProfileViewProps) {
        ...
        <div className="xl:col-span-8 space-y-6">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                            ...
                            <div className="text-xs text-slate-400 italic mt-2 relative pr-8">
                              <button
                                onClick={() => speak(selectedPersona.instruction)}
                                className={`absolute top-0 right-0 p-1 rounded-full transition-all duration-200 ${isSpeaking ? 'text-emerald-400 animate-pulse' : 'text-slate-500 hover:text-emerald-400'}`}
                                aria-label={`Listen to ${selectedPersona.name} instruction`}
                                disabled={isSpeaking}
                              >
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gradient-to-br from-slate-900/60 to-slate-900/40 backdrop-blur-md p-8 rounded-3xl border border-white/5 shadow-xl relative overflow-hidden group hover:border-indigo-500/30 transition-all duration-500 animate-fade-in">
                    <div className="absolute -right-8 -top-8 w-40 h-40 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none group-hover:bg-indigo-500/20 transition-all duration-500"></div>
                    
                    <div className="relative z-10 flex flex-col h-full justify-between">
                      <div>
                        <div className="flex items-center gap-3 mb-6">
                           <div className="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.15)] group-hover:scale-110 transition-transform">
                               <Lightbulb size={20} className="text-indigo-400" />
                           </div>
                           <h4 className="text-xs font-mono text-indigo-300/80 uppercase tracking-widest font-bold">Core Drive</h4>
                        </div>
                        <p className="text-3xl font-bold text-white mb-8 leading-tight drop-shadow-md pr-8">
                            {editedArch.coreDrive}
                            <button
                                onClick={() => speak(`Core Drive: ${editedArch.coreDrive}`)}
                                className={`absolute top-0 right-0 p-2 rounded-full transition-all duration-200 ${isSpeaking ? 'text-brand-accent animate-pulse' : 'text-slate-500 hover:text-indigo-400'}`}
                                aria-label={`Listen to core drive: ${editedArch.coreDrive}`}
                                disabled={isSpeaking}
                              >
                                {isSpeaking ? <VolumeX size={20} /> : <Volume2 size={20} />}
                            </button>
                      ...
                      {/* Reactor Animation */}
                      <div className="h-24 w-full bg-slate-950/50 rounded-2xl border border-white/5 relative overflow-hidden flex items-center justify-center group-hover:border-indigo-500/20 transition-colors">
                          ...
                          <div className="w-48 h-48 rounded-full border border-indigo-500/10 absolute animate-[spin_12s_linear_infinite]"></div>
                          <div className="w-32 h-32 rounded-full border border-indigo-400/20 absolute animate-[spin_8s_linear_infinite_reverse] border-dashed"></div>
                          <div className="w-3 h-3 rounded-full bg-indigo-400 shadow-[0_0_20px_rgba(99,102,241,1)] animate-pulse relative z-10"></div>
                      </div>
                    </div>
                  </div>

                  {/* Fuel Visualization */}
                  <div className="bg-gradient-to-br from-slate-900/60 to-slate-900/40 backdrop-blur-md p-8 rounded-3xl border border-white/5 shadow-xl relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-500 animate-fade-in" style={{ animationDelay: '100ms' }}>
                    <div className="absolute -right-8 -top-8 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none group-hover:bg-emerald-500/20 transition-all duration-500"></div>
                    
                    <div className="relative z-10 flex flex-col h-full justify-between">
                       <div>
                         <div className="flex items-center gap-3 mb-6">
                           <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.15)] group-hover:scale-110 transition-transform">
                               <Battery size={20} className="text-emerald-400" />
                           </div>
                           <h4 className="text-xs font-mono text-emerald-300/80 uppercase tracking-widest font-bold">Fuel Source</h4>
                        </div>
                        <p className="text-3xl font-bold text-white mb-8 leading-tight drop-shadow-md pr-8">
                            {editedArch.fuel}
                            <button
                                onClick={() => speak(`Fuel Source: ${editedArch.fuel}`)}
                                className={`absolute top-0 right-0 p-2 rounded-full transition-all duration-200 ${isSpeaking ? 'text-brand-accent animate-pulse' : 'text-slate-500 hover:text-emerald-400'}`}
                                aria-label={`Listen to fuel source: ${editedArch.fuel}`}
                                disabled={isSpeaking}
                              >
                                {isSpeaking ? <VolumeX size={20} /> : <Volume2 size={20} />}
                            </button>
                      ...
                      {/* Battery Bar */}
                       <div className="flex gap-2 h-24 items-end justify-center bg-slate-950/50 rounded-2xl border border-white/5 p-6 group-hover:border-emerald-500/20 transition-colors">
                          {[20, 40, 60, 80, 100].map((h, i) => (
                            <div key={i} className={`w-full rounded-sm transition-all duration-1000 ${i < 4 ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)] animate-shimmer' : 'bg-slate-800/50'}`} style={{ height: `${h}%` }}></div>
                          ))}
                        </div>
                    </div>
                  </div>
                        ...
                        <ul className="space-y-4">
                            {editedArch.bugs.length > 0 ? editedArch.bugs.map((bug, i) => (
                                <li key={i} className="bg-black/20 p-5 rounded-2xl border border-white/5 flex items-start gap-4 hover:border-amber-500/30 hover:bg-amber-500/5 transition-all group relative hover:-translate-x-0.5 hover:shadow-md">
                                    <button
                                        onClick={() => speak(`Bug ${i + 1}: ${bug}`)}
                                        className={`absolute top-2 right-2 p-1 rounded-full transition-all duration-200 ${isSpeaking ? 'text-emerald-400 animate-pulse' : 'text-slate-500 hover:text-amber-400'}`}
                                        aria-label={`Listen to bug ${i + 1}`}
                                        disabled={isSpeaking}
                                      >
                                        {isSpeaking ? <VolumeX size={16} /> : <Volume2 size={16} />}
                                    </button>
                        ...
                        <ul className="space-y-4">
                            {editedArch.patches.length > 0 ? editedArch.patches.map((patch, i) => (
                                <li key={i} className="bg-black/20 p-5 rounded-2xl border border-white/5 flex items-start gap-4 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all group relative hover:-translate-x-0.5 hover:shadow-md">
                                    <button
                                        onClick={() => speak(`Patch ${i + 1}: ${patch}`)}
                                        className={`absolute top-2 right-2 p-1 rounded-full transition-all duration-200 ${isSpeaking ? 'text-emerald-400 animate-pulse' : 'text-slate-500 hover:text-blue-400'}`}
                                        aria-label={`Listen to patch ${i + 1}`}
                                        disabled={isSpeaking}
                                      >
                                        {isSpeaking ? <VolumeX size={16} /> : <Volume2 size={16} />}
                                    </button>
            ...
            <div className="p-8 overflow-y-auto custom-scrollbar">
              <div className="grid md:grid-cols-2 gap-10">
                <div className="aspect-square bg-slate-950/50 rounded-full border border-slate-800 shadow-2xl p-4 flex items-center justify-center">
                  <NatalChart chart={associatedAstroChart} speak={speak} isSpeaking={isSpeaking} />
                </div>
